...
abstract class _HomeStoreBase with Store {
  ...
  Future<void> fetchMeasures() async {
    ...
    if (filter is DateFilterModel) {
      ...
      result = await repository.getMeasuresFromTrajectory(
        filter.trajectoryId,
        steps: min(
            steps, (filter.trajectory.duration.inMilliseconds / 100).round()),
      );
    } else {
  ...
  Future<bool> putTrajectoryName(String name) async {
    final traj = (filter as TrajectoryFilterModel?)?.trajectory;
    if (traj != null) return repository.putTrajectory(traj.id, name);
    return false;
  }
}
